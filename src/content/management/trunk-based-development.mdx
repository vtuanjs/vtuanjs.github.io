...
<Pipeline
  steps={[
    { label: 'fix in develop', note: 'always first', tone: 'accent' },
    { label: 'verify on staging' },
    { label: 'cherry-pick to production', tone: 'ok' },
  ]}
  caption="Fix production first and the next release silently overwrites the fix — the most common hotfix failure."
/>
