...
On a learning platform I worked on, we ran on feature branches: open one per feature, merge only when it was done. Branches stayed open for weeks and grew to thousands of changed files. Two people were quietly paying for that:

- **The manager had no visibility.** The system showed no change for weeks — no way to answer whether anything was actually getting done.
- **QA had no time.** The diff landed all at once right before release — one sprint to catch bugs in a month's worth of invisible change.
...
Once integration and release are separate:

- **The manager gets a running signal of progress** instead of a black box that resolves once every few weeks — merges to `develop` are visible immediately, even while the feature stays dark behind a flag.
- **QA gets its sprint back** — code lands in small pieces continuously instead of arriving as one release-week diff.
- **Product decides when to expose a feature** independently of when engineering finishes building it — nobody is stuck waiting on the other.
...
The common counter-objection: "That adds complexity." It does — one `if` statement per guarded feature. But the branch adds complexity too:

- Conflicts
- Merge coordination
- Review overload
- Delayed integration signal

**The question is which complexity is easier to reason about and clean up.** A flag with a clear owner is far easier to clean up than a branch that's fallen six weeks behind `develop`.
...
In practice it meant two codebases running in parallel:

- Fixes in production went into the release branch.
- If someone remembered, they also went into `develop`.
- Bugs appeared in production that had been fixed in `develop` but never backported.

**The release branch wasn't protecting production** — it was hiding integration debt and creating a second fix surface that diverged under pressure.
