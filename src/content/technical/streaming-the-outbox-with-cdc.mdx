...
Two implementation details are worth pulling out. `docker-compose.yml` sets `max_slot_wal_keep_size=1GB` on Postgres — a hard cap on WAL retained for the slot. Without it, a stalled Debezium takes the whole database down; with it, Postgres invalidates the slot instead, turning a catastrophic failure into a recoverable one. The recovery path is `make connector-recover`: delete the connector (clears the stored offset), drop the invalidated slot, re-register — no stored offset triggers a fresh snapshot on restart. The second detail: the connector sets `publication.autocreate.mode=disabled` and relies on `002_debezium.sql` to create the publication and the heartbeat table before the connector is ever registered. The `snapshot.mode: never` constraint from the trade-off table above is enforced by migration order, not left to convention.
