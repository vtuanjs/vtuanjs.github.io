...
- **Low-traffic database.** Pins the slot at an old position for hours — fixed with a **heartbeat**, having Debezium periodically write a tiny row so the slot advances.
- **`snapshot.mode: never`.** Does no backfill, so the slot must exist *before* the first outbox write or that first event is lost.
...
The full stack — Postgres, Kafka, Kafka Connect, Debezium 2.4, a Go order service, and a consumer — is at [github.com/vtuanjs/outbox](https://github.com/vtuanjs/outbox). Three commands get you to a live event flowing end-to-end:

```sh
make up                  # start Postgres + Kafka + Kafka Connect
make register-connector  # POST debezium/connector.json to Connect
make place-order         # write one order; watch the outbox row stream to Kafka
```

Two implementation details are worth pulling out:

- **`max_slot_wal_keep_size=1GB`** in `docker-compose.yml` — a hard cap on WAL retained for the slot. Without it, a stalled Debezium takes the whole database down; with it, Postgres invalidates the slot instead, turning a catastrophic failure into a recoverable one. Recovery is `make connector-recover`: delete the connector (clears the stored offset), drop the invalidated slot, re-register — no stored offset triggers a fresh snapshot on restart.
- **`publication.autocreate.mode=disabled`** — the connector relies on `002_debezium.sql` to create the publication and the heartbeat table before the connector is ever registered. The `snapshot.mode: never` constraint from the trade-off table above is enforced by migration order, not left to convention.
