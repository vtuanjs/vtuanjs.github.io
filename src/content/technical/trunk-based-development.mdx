...
On an LMS I worked on, we used feature branches: open one per feature, merge only when the feature was done. A branch would sit open for weeks and accumulate thousands of changed files. Two people paid for that in different ways. The manager had no visibility — the system showed no change for weeks, so there was no way to answer "did we actually do anything this sprint." QA had no time — a giant diff landed all at once, right before release, and they had one sprint to catch bugs in code that had been changing invisibly for a month.

Long-lived branches defer integration risk to the worst possible moment: right before release, in the largest possible batch. The fix isn't stricter branch hygiene — it's separating integration from release, so there's no reason to hold a branch open in the first place. We merge small changes to staging continuously with the flag on, and let preprod/prod run with the flag off — which buys the time to test and fix bugs before anyone flips the switch.
...
Every successful deploy to preprod or production creates a hotfix branch named after the deployed tag — `hotfixes/preprod/[tag]` and `hotfixes/prod/[tag]` — and stores those names in GitHub Actions variables (`HOTFIX_BRANCH_PREPROD`, `HOTFIX_BRANCH_PROD`). The branch name is derived from the tag's annotation message (`Branch: <branch-name>`), so when a hotfix itself gets deployed, post-deploy reads the annotation and resolves back to the original branch — the same branch is reused, not a new one per incident. A new branch is only created when a fresh release from `develop` is deployed.

The CI pipeline watches for pushes to `hotfixes/**`: `hotfixes/preprod/*` triggers build and auto-deploy to preprod; `hotfixes/prod/*` triggers build only — production stays manual.

This setup requires a `GetCurrentRelease` workflow action. Before cherry-picking, an engineer runs it to discover what's currently deployed: the action reads `HOTFIX_BRANCH_PREPROD` or `HOTFIX_BRANCH_PROD` from GitHub Actions variables and outputs the branch name along with the exact checkout and cherry-pick commands. Without this step there's no reliable way to know which hotfix branch to target — the variable is the single source of truth.
